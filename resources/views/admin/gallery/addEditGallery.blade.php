...
@php
   	if(isset($editGallery)){
   		$action = '/admin/gallery/'.$id;
   		$nameVal = $editGallery["title"];
   		$descVal = $editGallery["description"];
   		$contentVal = $editGallery["content"];
   		$published = ($editGallery["status"]==1)?'checked="checked"':'';
   		$unpublished = ($editGallery["status"]==0)?'checked="checked"':'';
   		$homepage = ($editGallery["homepage"]==1)?'checked="checked"':'';
   		$nonehome = ($editGallery["homepage"]==0)?'checked="checked"':'';
   		$imgsrc = '/images/gallery/thumbnails/'.$editGallery["image"];
	...
	<div class="container">
	  ...
	  <form method="post" enctype="multipart/form-data" action="{{ $action }}">
	    <div class="form-group row">
	      ...
	      @if(isset($editGallery))
	      	{{ method_field('patch') }}
	      	<input type="hidden" value = "{{$editGallery["id"]}}">
	      @endif
		...
		<div class="form-group row">
	      <label for="link" class="col-sm-2 col-form-label col-form-label-sm">Upload Video</label>
	      <div class="col-sm-8">
