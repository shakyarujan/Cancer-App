...
@php
   	if(isset($editNews)){
   		...
   		$nameVal = $editNews["title"];
   		$descVal = $editNews["description"];
   		$dateVal = $editNews["date"];
   		$authorVal = $editNews["author"];
   		$contentVal = $editNews["content"];
   		$published = ($editNews["status"]==1)?'checked="checked"':'';
   		$unpublished = ($editNews["status"]==0)?'checked="checked"':'';
   		$homepage = ($editNews["homepage"]==1)?'checked="checked"':'';
   		$nonehome = ($editNews["homepage"]==0)?'checked="checked"':'';
   		$imgsrc = '/images/news/thumbnails/'.$editNews["image"];

    }
    else{
    	$action = '/admin/news';
    	$nameVal = $descVal = $contentVal = $dateVal = $authorVal = '';
	...
	<div class="container">
	  ...
	  <form method="post" enctype="multipart/form-data" action="{{ $action }}">
	    <div class="form-group row">
	      ...
	      @if(isset($editNews))
	      	{{ method_field('patch') }}
	      	<input type="hidden" value = "{{$editNews["id"]}}">
	      @endif
	    ...
	    <div class="form-group row">
	      <label for="name" class="col-sm-2 col-form-label col-form-label-lg">Author</label>
	      <div class="col-sm-4">
	        <input type="text" class="form-control form-control-lg" id="author" placeholder="Author" name="author" value="{{$authorVal}}" required="required">
	      </div>

	      <label for="name" class="col-sm-1 col-form-label col-form-label-lg">Date</label>
	      <div class="col-sm-3">
	        <input type="text" class="form-control form-control-lg news_date" id="date" placeholder="Date" name="date" value="{{$dateVal}}" required="required">
	      </div>
	    </div>
